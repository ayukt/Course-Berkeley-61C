...
int P3colorpalette(char* colorfile, int width, int heightpercolor, char* outputfile)
{
	...
	FILE* in = fopen(outputfile, "w");
	if (in == NULL) {
		free(colorcount);
		printf("%s",outputfile);
		return 1;
	}
	if (fprintf(in, "%s %d %d %d\n", "P3", width, *colorcount * heightpercolor, 255) < 0) {
		free(colorcount);
		fclose(in);
		return 1;
	}
	...
	for (int i = 0; i < *colorcount; i++) {
		...
		for (int k = 0; k < heightpercolor; k++) {	
			for (int j = 0; j < width; j++) {
				if (fprintf(in, "%d %d %d", arr[0], arr[1], arr[2]) < 0 || 
				fprintf(in, j == width - 1 ? "\n" : " ") < 0) {
					free(colorcount);
					fclose(in);
					printf("d\n");
					return 1;
...
int P6colorpalette(char* colorfile, int width, int heightpercolor, char* outputfile)
{
	...
	if (fprintf(in, "%s %d %d %d\n", "P6", width, *colorcount * heightpercolor, 255) < 0) {
		free(colorcount);
		fclose(in);
		return 1;
	}
	...
	for (int i = 0; i < *colorcount; i++) {
		...
		for (int k = 0; k < heightpercolor; k++) {	
			for (int j = 0; j < width; j++) {
				fwrite(arr, sizeof(uint8_t), 3, in);
			}
		}
	}
	free(colorcount);
